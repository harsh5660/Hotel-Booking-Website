...
if(isset($_POST['delete'])){
   ...
   $delete_id = filter_var(value: $delete_id, filter: FILTER_SANITIZE_STRING);

   $verify_delete = $conn->prepare(query: "SELECT * FROM `bookings` WHERE booking_id = ?");
   $verify_delete->execute(params: [$delete_id]);

   if($verify_delete->rowCount() > 0){
      $delete_bookings = $conn->prepare(query: "DELETE FROM `bookings` WHERE booking_id = ?");
      $delete_bookings->execute(params: [$delete_id]);
