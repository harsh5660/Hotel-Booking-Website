...
if(isset($_POST['cancel'])){
   ...
   $booking_id = filter_var(value: $booking_id, filter: FILTER_SANITIZE_STRING);

   $verify_booking = $conn->prepare(query: "SELECT * FROM `bookings` WHERE booking_id = ?");
   $verify_booking->execute(params: [$booking_id]);

   if($verify_booking->rowCount() > 0){
      $delete_booking = $conn->prepare(query: "DELETE FROM `bookings` WHERE booking_id = ?");
      $delete_booking->execute(params: [$booking_id]);
