...
if(isset($_POST['check'])){
   ...
   $check_in = filter_var(value: $check_in, filter: FILTER_SANITIZE_STRING);

   $total_rooms = 0;

   $check_bookings = $conn->prepare(query: "SELECT * FROM `bookings` WHERE check_in = ?");
   $check_bookings->execute(params: [$check_in]);

   while($fetch_bookings = $check_bookings->fetch(mode: PDO::FETCH_ASSOC)){
...
if(isset($_POST['book'])){
   ...
   $number = $_POST['number'];
   $number = filter_var(value: $number, filter: FILTER_SANITIZE_STRING);
   $rooms = $_POST['rooms'];
   $rooms = filter_var(value: $rooms, filter: FILTER_SANITIZE_STRING);
   $check_in = $_POST['check_in'];
   $check_in = filter_var(value: $check_in, filter: FILTER_SANITIZE_STRING);
   $check_out = $_POST['check_out'];
   $check_out = filter_var(value: $check_out, filter: FILTER_SANITIZE_STRING);
   $adults = $_POST['adults'];
   $adults = filter_var(value: $adults, filter: FILTER_SANITIZE_STRING);
   $childs = $_POST['childs'];
   $childs = filter_var(value: $childs, filter: FILTER_SANITIZE_STRING);
...
if(isset($_POST['send'])){
   ...
   $phonenumber = filter_var(value: $phonenumber, filter: FILTER_SANITIZE_STRING);
   $message = $_POST['message'];
   $message = filter_var(value: $message, filter: FILTER_SANITIZE_STRING);

   $verify_message = $conn->prepare("SELECT * FROM `messages` WHERE name = ? AND email = ? AND number = ? AND message = ?");
   $verify_message->execute([$name, $email, $phonenumber, $message]);

   if($verify_message->rowCount() > 0){
      $warning_msg[] = 'message sent already!';
   }else{
      $insert_message = $conn->prepare("INSERT INTO `messages`(id, name, email, number, message) VALUES(?,?,?,?,?)");
      $insert_message->execute([$id, $name, $email, $phonenumber, $message]);
...
<section class="gallery" id="gallery">
   ...
   <div class="swiper gallery-slider">
      <div class="swiper-wrapper">
         ...
         <img src="images/hero1.png" class="swiper-slide" alt="">
      </div>
...
<section class="contact" id="contact">
   ...
   <div class="row">
      ...
      <form action="" method="post">
         ...
         <input type="phone-number" name="phone-number" required maxlength="10" min="0" max="9999999999" placeholder="enter your number" class="box">
